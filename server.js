/**
 * server.js — Land Bot combined entry point
...
app.get("/", (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html><head><title>Land Bot</title>
...
async function runOutreach() {
  ...
  console.log(`\n${"=".repeat(55)}`);
  console.log(`  LAND FLIPPING BOT — SCHEDULED RUN`);
  ...
  if (!leads.length) {
    ...
    for (const lead of leads) {
      ...
      let analysis;
      try {
        analysis = await m.analyzeLand(lead);
      } catch (err) {
      ...
      if (!analysis.ourOffer || analysis.ourOffer <= 0) {
        m.addLead(log, { ...lead, analysis, skipReason: "no offer calculated" });
        m.saveLog(log);
        continue;
      }
      ...
      if (!DRY_RUN) {
        ...
        // Also email if skip trace found an address (double touch = more responses)
        if (lead.email) {
          try {
  ...
  console.log(`\n📞 Running RVM batch (existing leads without voicemail)...`);
  try {
    const rvmSent = await m.runRVMBatch(DRY_RUN);
    if (rvmSent > 0) {
      _stat("rvmSent", rvmSent);
      _stats.lastOutreachAt = new Date().toISOString();
    }
  } catch (err) {
    console.error(`⚠️  RVM batch error: ${err.message}`);
  }

  try {
    const cities = ["austin", "nashville", "charlotte", "tampa", "houston"];
    const city = cities[Math.floor(Math.random() * cities.length)];
    await m.updateBuilderDatabase(city);
  } catch (err) {
...
// ── Cron: every 2 hours, 8am–8pm ET, every day ───────────────────────────────
...
console.log("📅 Cron scheduled: every 2 hours, 8am–8pm ET, daily (7 runs/day)");
